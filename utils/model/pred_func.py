...
def pred_vid(df, model):
    with torch.no_grad():
        # If needed, consider removing torch.sigmoid if your model outputs logits.
        return max_prediction_value(torch.sigmoid(model(df).squeeze()))
...
def max_prediction_value(y_pred):
    ...
    mean_val = torch.mean(y_pred, dim=0)
    # Convert the averaged values into a probability distribution
    probabilities = torch.softmax(mean_val, dim=0)
    # Get predicted class (index) and its confidence (max probability)
